...
namespace FinniganFileIO
{
	...
	public class XRawFileIO : FinniganFileReaderBaseClass, IDisposable
	{
		...
		public static void ExtractMRMMasses(string strFilterText, MRMScanTypeConstants eMRMScanType, out udtMRMInfoType udtMRMInfo)
		{
			...
			InitializeMRMInfo(out udtMRMInfo, 0);
			...
			if (!string.IsNullOrWhiteSpace(strFilterText)) {
				...
				if (eMRMScanType == MRMScanTypeConstants.MRMQMS | eMRMScanType == MRMScanTypeConstants.SRM) {
					...
					if (reMatch.Success) {
						...
						if ((reMatch != null)) {
							InitializeMRMInfo(out udtMRMInfo, 2);
		...
		private override bool FillFileInfo()
		{
...
throw new NotImplementedException();
					...
					for (intIndex = 0; intIndex <= intMethodCount - 1; intIndex++) {
						strMethod = null;
						mXRawFile.GetInstMethod(intIndex, strMethod);
						if (string.IsNullOrWhiteSpace(strMethod)) {
							mFileInfo.InstMethods[intIndex] = string.Empty;
						} else {
							mFileInfo.InstMethods[intIndex] = string.Copy(strMethod);
						}
		...
		private ActivationTypeConstants GetActivationType(int scan, int msLevel)
		{
			...
			try {
				...
				ActivationTypeConstants activationType = default(ActivationTypeConstants);

				if (!Enum.TryParse<ActivationTypeConstants>(activationTypeCode.ToString(), out activationType)) {
					activationType = ActivationTypeConstants.Unknown;
				}
		...
		public override bool GetScanInfo(int scan, out clsScanInfo scanInfo)
		{
			...
			try {
				...
				mXRawFile.GetScanHeaderInfoForScanNum(scan, scanInfo.NumPeaks, scanInfo.RetentionTime, scanInfo.LowMass, scanInfo.HighMass, scanInfo.TotalIonCurrent, scanInfo.BasePeakMZ, scanInfo.BasePeakIntensity, scanInfo.NumChannels, intBooleanVal,
				scanInfo.Frequency);
				...
				if (intArrayCount > 0) {
					string[] scanEventNames = null;
					string[] scanEventValues = null;

					scanEventNames = Convert.ToString(objLabels);
					scanEventValues = Convert.ToString(objValues);
				...
				if (scanInfo.MRMScanType != MRMScanTypeConstants.NotMRM) {
					...
					InitializeMRMInfo(out newMRMInfo, 0);
				}
				...
				if (intArrayCount > 0) {
					...
					logNames = Convert.ToString(objLabels);
					logValues = Convert.ToString(objValues);
		...
		private void GetTuneData()
		{
			...
			for (var intIndex = 0; intIndex <= intNumTuneData - 1; intIndex++) {
				...
				if (intTuneLabelCount > 0)
				{
					...
					if (intTuneLabelCount > 0) {
						var strTuneSettingNames = Convert.ToString(objLabels);
						var strTuneSettingValues = Convert.ToString(objValues);
		...
		private udtScanHeaderInfoType ScanInfoClassToStruct(clsScanInfo scanInfo)
		{
		    ...
		    var udtScanInfo = new udtScanHeaderInfoType()
		    {
		        ...
		        ScanEvents = scanInfo.ScanEvents,
		        StatusLog = scanInfo.StatusLog
		    };
		...
		public int GetScanData2D(int scan, out double[,] dblMassIntensityPairs, int intMaxNumberOfPeaks, bool blnCentroid)
		{
			...
			try {
				...
				if (blnCentroid && scanInfo.IsFTMS) {
					...
					double[,] dblMassIntensityLabels = null;

					dblMassIntensityLabels = Convert.ToDouble(massIntensityLabels);
					...
					if (dataCount > 0) {
						dblMassIntensityPairs = Convert.ToDouble(MassIntensityPairsList);
					} else {
						dblMassIntensityPairs = new double[0, 0];
					}
		...
		public int GetScanLabelData(int scan, out udtFTLabelInfoType[] ftLabelData)
		{
			...
			try {
				...
				double[,] labelDataArray = null;
				labelDataArray = Convert.ToDouble(labelData);
		...
		public int GetScanPrecisionData(int scan, out udtMassPrecisionInfoType[] massResolutionData)
		{
			...
			try {
				...
				double[,] massPrecisionArray = null;
				massPrecisionArray = Convert.ToDouble(massResolutionDataList);
		...
		/// </summary>
		/// <remarks></remarks>
		public XRawFileIO() : this(string.Empty)
		{
		}
