...
public class clsScanInfo
{
	...
	public bool TryGetScanEvent(string eventName, out string eventValue, bool partialMatchToStart = false)
	{
		...
		if (partialMatchToStart) {
			// Partial match
			lstResults = from item in mScanEventswhere item.Key.ToLower().StartsWith(eventName.ToLower())item;
		} else {
			lstResults = from item in mScanEventswhere string.Equals(item.Key, eventName, StringComparison.InvariantCultureIgnoreCase)item;
		}

		foreach (void item_loopVariable in lstResults) {
