    ...
    public static class Program
    {
        ...
        private static void ExtractScanFilters(string directoryToScan)
        {
            ...
            else
            {
                ...
                if (defaultFile.Directory?.Exists == true)
                {
                    ...
                    if (alternateScanStatsFiles.Count > 0)
                    {
                        ...
                        Console.WriteLine("Parsing _ScanStatsEx.txt files in directory " +
                                          PathUtils.CompactPathString(defaultFile.Directory.FullName, 75));
                    }
                }
            }
            ...
            foreach (var dataFile in scanStatsFiles)
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    foreach (var match in parentIonMZnoAtMatcher.Matches(scanFilterGeneric).Cast<Match>())
                    {
                        scanFilterGeneric = scanFilterGeneric.Replace(match.Value, match.Groups[1].Value + " 0.000");
                    }

                    // Update collision energies
                    foreach (var match in collisionModeMatcher.Matches(scanFilterGeneric).Cast<Match>())
                    {
        ...
        private static void TestReader(string rawFilePath, bool centroid = false, bool testSumming = false, int scanStart = 0, int scanEnd = 0)
        {
            try
            {
                ...
                for (var scanNum = scanStart; scanNum <= scanEnd; scanNum += scanStep)
                {
                    ...
                    {
                        ConsoleMsgUtils.ShowWarning("Exiting for loop since scan number {0} is greater than the max scan number, {1}", scanNum,
                            reader.ScanEnd);
        ...
        private static void TestGetAllScanEvents(string rawFilePath)
        {
            try
            {
                ...
                // Values are dictionaries tracking all the values for each event (key is value, value is occurrence of that value)
